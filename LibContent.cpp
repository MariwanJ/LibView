...
int CLibContent::ExtractCppSymbol(char* szDecoratedName, DWORD dwLen)
{
	...
	nRet = UnDecorateSymbolName(szDecoratedName, szFunc, 1024, UNDNAME_COMPLETE | UNDNAME_32_BIT_DECODE);
	if (!nRet)
		return GetLastError();
	...
	return  m_cMemStore.Add(std::string(szDecoratedName), std::string(szFunc));
}
...
int CLibContent::GetFuncDetail(DWORD dwIndex, std::string& szDecorated, std::string& szUndecorated)
{
	std::string s1, s2;
...
{
	return m_cMemStore.m_ppstInfo.size();
}